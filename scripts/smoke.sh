...
#   bash scripts/smoke.sh prod           # 生产: 走 nginx 域名 (https://work.ibi.ren + https://admin.work.ibi.ren)
#   bash scripts/smoke.sh ecs            # 兼容: = prod (旧"ecs"模式已废,8080/8081 不再暴露)
#   bash scripts/smoke.sh <url>          # 自定义 base URL
#
# 域名配置走 deploy.env:
#   PROD_WEB_DOMAIN=https://work.ibi.ren              (默认,前台)
#   PROD_ADMIN_DOMAIN=https://admin.work.ibi.ren      (默认,后台,独立子域)
#
# 检查项:
...
case "$TARGET" in
  ...
  prod|ecs)
    ...
    ADMIN_DOMAIN="${PROD_ADMIN_DOMAIN:-https://admin.work.ibi.ren}"
    BASE="$WEB_DOMAIN"
