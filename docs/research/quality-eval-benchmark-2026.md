...
> **关联记忆**:[[project-ai-quality-eval-4layers]] / [[project-line-item-catalog-tier]] / [[project-w25-ai-eval-research-done]]
...
**W2.5 MVP 走"全跑 4 层 + Claude 主用 + 阿里云合规 + 不开 GPU":L1 ffprobe(免费)+ L2 美学 Claude Sonnet 4.5(¥0.118/条,VideoAesBench 67.88% 第 1)+ L3 阿里云内容安全增强版(~¥552/月)+ L4 商业价值同 Claude(¥0.118/条)。月成本 ¥2,445,在 ¥1,145 现状上加 ¥1,300,不动 ECS,3 天上线**。
...
- **用户决策风格(2026-07-02 拍板)**:质量优先于成本 — 选了"全跑 4 层 + Claude 主 + 极致分层抽审"组合,放弃"国产替代 + 分层调用"省钱的推荐项。
- **关键风险**:Claude API 海外调用受国内合规要求,**必须走 Anthropic 官方 + 严格 PII 脱敏**(见 §9.6 免责声明)。
...
### 3.3 推荐(2026-07-02 拍板):主用 Claude Sonnet 4.5,Qwen 备用

> **决策依据**:用户选"全跑 4 层 + 质量优先"路线,放弃国产替代省 ¥700/月的推荐。

| 角色 | VLM | 调用方式 | 单条 30s 视频成本 |
|---|---|---|---|
| **主用(默认全跑)** | **Claude-Sonnet-4.5** | Anthropic API + Structured Outputs | 8 帧 ≈ ¥0.118(L2)+ ¥0.118(L4)= ¥0.236/视频 |
| **国内 fallback** | **Qwen-VL-Max** | 阿里云百炼 OpenAI 兼容 API | 8 帧 ≈ ¥0.006 |
...
3. **批处理**:Claude Batch API 50% off(24h 内出结果),用于非实时评分。
4. **冷启动校准**:跑 AVA(200 张)+ KoNViD-1k(50 段)出 PLCC/SRCC baseline,再跑平台自家 AIGC 数据。
5. **数据出境合规**:Claude 调用须 PII 脱敏(详见 §9.6 免责声明),避免违规传输用户身份信息。
...
### 7.3 三个方案月成本对比(2026-07-02 拍板后场景)

> **假设(2026-07-02 拍板)**:L1+L2+L3+L4 全跑,L2/L4 主用 Claude Sonnet 4.5(¥0.118/次),L3 阿里云增强版(¥0.55/条),不自建 GPU。
> - 单条视频 VLM 成本 = L2 ¥0.118 + L4 ¥0.118 = **¥0.236**
> - 月 VLM 总成本 = 100 条/天 × 30 天 × ¥0.236 = **¥708**

| 方案 | VLM 评审 | 内容安全 | GPU | OSS | 基线 | **合计** |
|---|---|---|---|---|---|---|
| **方案 A — 全 API + Claude 全跑(拍板)** | ¥708 | ¥552 | ¥0 | ¥40 | ¥1,145 | **¥2,445/月** |
| 方案 B — Claude + 1 块 T4(FAST-VQA) | ¥708 | ¥552 | ¥1,681 | ¥40 | ¥1,145 | **¥4,126/月** |
| 方案 C — Claude + T4(口型 + VQA) | ¥708 | ¥552 | ¥1,681 | ¥40 | ¥1,145 | **¥4,126/月** |
| 方案 D — Claude + Qwen 国产 fallback(W3+ 评估) | ¥708 | ¥552 | ¥0 | ¥40 | ¥1,145 | **¥2,445/月** |
| ~~方案 A' — 全 API + Qwen + 分层(原推荐,未采纳)~~ | ~~¥108~~ | ~~¥552~~ | ¥0 | ~~¥40~~ | ~~¥1,145~~ | ~~¥1,845/月~~ |

**方案 A(拍板)优点**:质量最优(VideoAesBench 第 1),3 天上线。**缺点**:月成本 ¥2,445,比原推荐贵 ¥600。

**方案 B/C 优点**:美学准确率再 +25%。**缺点**:GPU 单占 ¥1,681,W2.5 不划算(用户拍板不上 GPU)。

### 7.4 省钱招(方案 A 应用版)

> 用户拍板"全跑 4 层 + Claude 主",部分省钱招不再适用,以下保留仍有效的:

| 招数 | 节省 | 是否仍用 | 实现 |
|---|---|---|---|
| **QPS 削峰** | API 成本 -20% | ✅ 仍用 | 评审任务排队,避开 Anthropic 高峰(UTC 22:00-02:00) |
| **抽帧降本** | VLM token -75% | ✅ 仍用 | 5 分钟视频抽 8 帧 → 仅 1500 token |
| **Claude Batch API** | -50% | ✅ 仍用 | 输入输出均 50% 折扣(24h 出结果);非实时评分全走 Batch |
| **Prompt Caching** | -75% system prompt | ✅ 仍用 | 评分模板固定,system prompt 缓存可大省 |
| **分层调用** | -60% | ❌ 不采纳 | 用户拍板全跑 4 层 |
| **国产替代** | -98% | ❌ 不采纳 | 用户拍板 Claude 主,Qwen 仅作 fallback |
| **OSS 临时存储** | -42% | ✅ 仍用 | 评审素材用临时存储(¥0.07 vs ¥0.12/GB/月) |
| **GLM-4V-Flash 免费初筛** | ¥0/条 | ⚠️ 部分用 | premium 档直接 Claude,essential 档可走 GLM 初筛 |

**应用后方案 A 实际成本估算**:
- VLM 评审:¥708/月 × 0.5(Batch+Cache) = **¥354/月**
- 内容安全:¥552/月
- OSS:¥40/月
- 基线:¥1,145/月
- **实际合计**:**¥2,091/月**(理论 ¥2,445 应用优化后节省 ¥354)
- 加上争议预留(月 10% dispute 重跑 Claude):+ ¥71/月
- **优化后稳态合计**:**¥2,162/月**
...
### 7.6 推荐路径(2026-07-02 拍板后)

**短期(W2.5 上线,2 周)** — 已拍板:
1. 走 **方案 A 拍板版**(全 API + Claude 全跑 + Batch + Cache 优化)
2. 月成本 **¥2,445**(应用 Batch+Cache 后稳态 ¥2,162),在 ¥1,145 现状上加 ¥1,017~1,300
3. 不动 ECS,3 天能上线

**中期(W3-W4,1 个月,需重新评估)**:
1. 加 `gn6i` T4(¥1,681/月)跑 FAST-VQA,出方案 B(月 ¥4,126)
2. 或评估"Qwen fallback 切换"节省:Claude 限流或涨价时启用 Qwen
3. 美学准确率从 VLM-only 67.88% 升到 85%

**长期(W8+,3 个月)**:
1. 攒够平台 AIGC 人评数据(200+ 条),做"VLM vs 人评"SRCC 校准
2. 评测 GLM-4V-Plus 是否能替代 Claude(国内合规 + 成本优势)
3. 引入 DOVER-Mobile(9.86M 参数)做轻量视频质量基线
4. OSS 归档评审素材到低频存储(¥0.024/GB/月)
...
### 8.2 综合技术选型(W2.5 MVP,2026-07-02 拍板版)

| 层 | 主用 | 备用 | 算力 | 单条成本 |
|---|---|---|---|---|
| **L1 技术质量** | ffprobe + ffmpeg + opencv | — | **本地 CPU**(免费) | ¥0 |
| **L2 美学质量** | **Claude Sonnet 4.5**(主用)+ AesV2.5(抽 8 帧锚) | Qwen-VL-Max(国内 fallback)/ GLM-4V-Flash(初筛免费) | Anthropic API + 本地 CPU | ¥0.118/条 |
| **L3 合规质量** | **阿里云内容安全增强版** | 腾讯云天御 + 自建广告法词表 | API | ¥0.55/条(1000 brief/月) |
| **L4 商业价值** | Claude Sonnet 4.5(同 L2,prompt 不同) | Qwen(国内 fallback) | Anthropic API | ¥0.118/条 |
| **单条总成本** | | | | **¥0.785/条** |
| **月总成本** | | | | **¥2,445/月**(Batch+Cache 后稳态 ¥2,162) |

**融合公式**:`总分 = L1 × 0.15 + L2 × 0.30 + L3 × 0.25 + L4 × 0.30`,闸门 L3=0 一票否决。

**分级**:≥ 0.85 = S / 0.70-0.85 = A / 0.60-0.70 = B / < 0.60 = C。

**S/A/B/C ↔ SKU 档位映射**(拍板后待定):
- 评分 S → premium 档(¥2,500-3,500)准入
- 评分 A → standard 档(¥1,400-2,000)准入
- 评分 B → essential 档(¥700-1,000)准入
- 评分 C → 不上架,创作者重做
- **注意**:这是建议,**S/A/B/C 与 essential/standard/premium 的具体映射规则需另立评审矩阵**(关联 [[project-line-item-catalog-tier]])

### 8.3 W2.5 实施里程碑(2 周,2026-07-02 拍板版)

| 天 | 任务 | 交付物 |
|---|---|---|
| D1 | 阿里云内容安全增强版开通 + 资源包购买 + QPS 预留包 + **Anthropic API key 配置** | `aliyun-moderation.ts` provider |
| D2 | NestJS moderation module + Service 接口层(主备)+ VLM gateway(Claude 主,Qwen fallback) | `apps/api/src/moderation/` + `apps/api/src/vlm/` |
| D3 | 广告法词表自建 fallback(JSON,1000 元/年买词表)+ PII 脱敏 helper(给 Claude 用) | `keywords/ad-law-words.json` + `pii-redactor.ts` |
| D4 | **Claude Sonnet 4.5 接入 + Anthropic Structured Outputs + AesV2.5 抽帧** | `vlm-judge.service.ts` |
| D5 | 评分流水线主流程(**L1+L2+L3+L4 全跑**)+ Batch API 配置 | `quality-eval.service.ts` |
| D6-D7 | Evidence 落库 + 评分 JSON 输出 + 法务免责声明上线 | DB schema + Audit log + `/legal/ai-disclaimer` |
| D8-D9 | admin 端"质量评审"页面 + **W2 #30 漏的买家 bid 列表 + accept 按钮**(1 天) | admin `/quality-eval` + web buyer 页 |
| D10 | 校准集脚本(AVA + KoNViD-1k)+ Anthropic Batch 任务投递脚本 | `scripts/eval-vlm-bench.sh` |
| D11-D12 | 攒人评 + 跑 SRCC 校准(Claude vs 人评) | `docs/research/vlm-calibration-2026-07.md` |
| D13 | 平台 A/B 切流 50/50(原 1.0 vs 新 2.0) | admin 切流开关 |
| D14 | review + 修复 + 上线 | 全流程跑通 |

**总计 13-14 天**(已加 2 天缓冲)。W2 #30 漏掉的"买家 bid 列表 + accept 按钮"插在 D8-D9 同窗口并行做。

**Anthropic API 接入注意事项**(D1 必读):
- 国内 ECS 调用 Anthropic 需走专线/反向代理,确认 ECS 出境网络通畅
- prompt 必须先过 PII 脱敏(`pii-redactor.ts` 替换姓名/手机号/邮箱)
- 启用 Batch API(50% off,24h 内出)用于非实时评分
- 启用 Prompt Caching(75% off system prompt)用于评分模板

### 8.4 风险与缓解(综合)

| 风险 | 概率 | 影响 | 缓解 |
|---|---|---|---|
| VLM 美学偏见(西方 vs 中国短视频) | 高 | L2 失真 | 校准集 + Qwen fallback + 国产 VLM 对比 |
| **Anthropic API 国内可访问性** | **中** | **评分中断** | Qwen fallback + Batch 异步 + ECS 出境监控 |
| **数据出境合规** | **中** | **监管风险** | PII 脱敏 + 法务免责声明(§9.6) |
...
## 9. 已拍板决策记录(2026-07-02)

> **结论先行**:12 项决策全部拍板,W2.5 MVP 走"全跑 4 层 + Claude 主 + 阿里云合规 + 不开 GPU"质量优先路线。月成本 ¥2,445(Batch+Cache 后稳态 ¥2,162),不动 ECS,3 天上线。

### 9.1 产品方向(7 项)

| # | 决策项 | 用户决定 | 备注 |
|---|---|---|---|
| 1 | 评分公开范围 | **买家 + 创作者都看** | evidence 必现(每条扣分带截图) |
| 2 | 权重是否可调 | **平台固定** L1×0.15 + L2×0.30 + L3×0.25 + L4×0.30 | 跨 brief 可比 |
| 3 | 评分周期 | **每次交付评**(默认) | 一次性/月度可后续扩展 |
| 4 | 评分形态 | **S / A / B / C 4 档分级** | ≥0.85=S / 0.70-0.85=A / 0.60-0.70=B / <0.60=C |
| 5 | 3 档人工抽审 | **essential 0% / standard 10% / premium 100%**(极致分层) | 人工成本最低,premium 零失误 |
| 6 | 评分申诉机制 | **每条 1 次,SLA 48h** | 复审触发 Claude 重跑(免费) |
| 7 | 法务免责声明 | **见 §9.6 草案** | 待法务 review 后落地 |

### 9.2 技术方向(5 项)

| # | 决策项 | 用户决定 | 备注 |
|---|---|---|---|
| 8 | 分层调用省钱 | **❌ 不采纳,L1+L2+L3+L4 全跑**(用户选) | 月成本 +¥130,但首版体感最佳 |
| 9 | VLM 主选 | **Claude Sonnet 4.5 全部**(用户选) | 搭配"全跑"路线,质量优先 |
| 10 | T4 GPU | **W2.5 不开**(推荐) | W3+ 重新评估 |
| 11 | 内容安全云 API | **阿里云增强版 ★★★★★** | aigcCheck + aigcViolationDetection + ad_compliance_detection |
| 12 | W2 #30 同期上线 | **✅ 同期,1 天并到 D8-D9** | 买家 bid 列表 + accept 按钮 |

### 9.3 决策风格观察

> 用户决策一致偏向**"质量优先 + 完整闭环"**:
> - 评分形态:4 档分级(而非二元通过/不通过)
> - 评分公开:买家 + 创作者都看(而非 admin 才看)
> - 抽审比例:0% / 10% / 100%(而非统一 30%)
> - 分层调用:全跑 4 层(而非仅 dispute 时跑)
> - VLM:Claude 主(而非 Qwen 国产替代)
>
> **推论**:用户认为 MVP 阶段不值得用"省钱招"换"体感",先把质量做出来,W3+ 有了真实数据再优化成本。这与 [[project-positioning-ip-marketplace]] 的"短期活下去优先"略有张力 — 但评分作为对外公信力展示面,确实应优先质量。

### 9.4 决策变更带来的成本影响

| 项 | 原推荐方案 | 用户拍板方案 | 差额 |
|---|---|---|---|
| VLM 成本 | ¥108/月(Qwen + 分层) | ¥708/月(Claude 全跑) | **+¥600** |
| 内容安全 | ¥110/月(基础版) | ¥552/月(增强版) | +¥442 |
| 其他(OSS/削峰) | ¥40 | ¥40 | 0 |
| **月总成本** | **¥1,403/月** | **¥2,445/月** | **+¥1,042(+74%)** |

**对比基线 ¥1,145**:
- 原推荐:加 ¥258(+22%)
- 拍板方案:加 ¥1,300(+114%)

**决策合理性**:月 ¥1,300 增量换"全跑 4 层 + Claude 主 + 极致分层抽审",质量上限提升明显,符合 MVP 阶段质量优先策略。

### 9.5 后续可调节点(W3+ 重评估)

| 项 | 何时重新评估 | 评估指标 |
|---|---|---|
| VLM 是否切 Qwen | Claude API 国内可访问性出问题,或成本超 ¥1,000/月 | 月度账单 + 评分申诉率 |
| 是否开 T4 GPU | L2 美学申诉率 >10%,或平台 AIGC 累计 1000+ 条 | 申诉率 + PLCC/SRCC |
| 抽审比例 | premium 档误判率 >5%,或 essential 档投诉率 >2% | 误判率 + 投诉率 |
| S/A/B/C ↔ SKU 映射 | 评分上线 1 个月后,看分布 | 评分分布直方图 |

### 9.6 法务免责声明草案

> **状态**:草案,**待法务 review 后落地**。W2.5 上线前必须由法务签字。

```
【AI 自动评分免责声明 — 草案 v0.1】

1. AI 评分仅供参考,不构成质量保证或合同标的。
   本平台 AI 评分系统(以下简称"评分系统")基于多模态大模型(Claude Sonnet 4.5 / Qwen-VL-Max / 阿里云内容安全 API)对创作者交付物进行技术、美学、合规、商业价值 4 个维度的自动化评估。

2. 评分系统可能存在以下局限:
   (a) 对 AIGC 长尾美感(动漫风/2.5D/虚拟人)的判断可能存在偏见;
   (b) 中文 prompt 偶发省略关键约束;
   (c) 长视频抽帧可能丢失细节;
   (d) 阿里云内容安全 API 对 AIGC 内容可能误判;
   (e) 跨境 API 调用(Anthropic)受国内合规要求约束,仅传输经 PII 脱敏的图像帧。

3. 创作者对评分有异议的,可在评分公布后 48 小时内提交 1 次申诉,本平台承诺:
   (a) 申诉由独立于初评的运营人员复审;
   (b) 复审期间启用 Claude 重跑(独立证据链);
   (c) 复审结论为最终结论,不再二次申诉。

4. AI 评分**不得**作为以下用途的唯一依据:
   (a) 司法/仲裁证据;
   (b) 投资决策依据;
   (c) 保险理赔依据。
...
5. 本平台对以下情形不承担责任:
   (a) 因 AI 评分错误导致的创作损失;
   (b) 因跨境 API 中断导致评分延迟;
   (c) 因内容安全 API 误判导致的临时下架(已建立误判申诉通道)。

6. 本免责声明的解释权归 ibi.ren 平台所有。

【联系方式】
法务邮箱:legal@ibi.ren(待开通)
争议处理 SLA:48 小时
```
