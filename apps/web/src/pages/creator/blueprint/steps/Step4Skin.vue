...
if (!ctx) throw new Error('Step4Skin must be inside BlueprintWizard (BlueprintKey not provided)');
const blueprintCtx = ctx;

const form = ref<L4Skin>({ ...L4_DEFAULTS });
const saving = ref(false);
const lastSavedAt = ref<string | null>(null);

const draft = useBlueprintDraft<L4Skin>({
  blueprintId: props.blueprintId,
  step: 4,
  serverData: ref<L4Skin | null>(null),
  formData: form,
});
...
watch(
  () => blueprintCtx.blueprint.value?.layers.L4_skin,
  (serverLayer) => {
    if (!serverLayer || draftRestored.value) return;
    form.value = { ...form.value, ...(serverLayer as unknown as L4Skin) };
  },
  { immediate: true },
);
...
async function persist() {
  ...
  saving.value = true;
  try {
    await blueprintCtx.updateLayer(4, form.value as unknown as Record<string, unknown>);
...
const isLayerInferred = useStepInferred(blueprintCtx, 'L4_skin');
</script>
  ...
  <article class="paper-grain rounded-md border border-ink/15 bg-cream p-6 shadow-paper">
    <header class="mb-4 border-b border-ink/10 pb-3">
      <div class="flex items-center justify-between">
        <div>
          <p class="font-mono text-xs uppercase tracking-widest text-ink/40">L4 / 6</p>
          <h2 class="font-display text-2xl text-ink">皮肤 (Skin)</h2>
    ...
    <section class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
      <div v-for="f in L4_SELECT_FIELDS" :key="f.key">
    ...
    <section class="space-y-4">
      <div v-for="f in L4_SLIDER_FIELDS" :key="f.key">
