...
/**
 * Step 6 — L6 修饰 (Decoration)
 *
 * 6 项:妆容 / 唇色 / 腮红 / 眼影 / 装饰 / 面部彩绘
 * 数据来源:BlueprintContext
 */
...
if (!ctx) throw new Error('Step6Decoration must be inside BlueprintWizard (BlueprintKey not provided)');
const blueprintCtx = ctx;

const form = ref<L6Decoration>({ ...L6_DEFAULTS });
const saving = ref(false);
const lastSavedAt = ref<string | null>(null);

const draft = useBlueprintDraft<L6Decoration>({
  blueprintId: props.blueprintId,
  step: 6,
  serverData: ref<L6Decoration | null>(null),
  formData: form,
});
...
watch(
  ...
  (serverLayer) => {
    ...
    form.value = { ...form.value, ...(serverLayer as unknown as L6Decoration) };
  },
  { immediate: true },
);
...
async function persist() {
  ...
  saving.value = true;
  try {
    await blueprintCtx.updateLayer(6, form.value as unknown as Record<string, unknown>);
...
const isLayerInferred = useStepInferred(blueprintCtx, 'L6_decoration');
</script>
  ...
  <article class="paper-grain rounded-md border border-ink/15 bg-cream p-6 shadow-paper">
    <header class="mb-4 border-b border-ink/10 pb-3">
      <div class="flex items-center justify-between">
        <div>
          <p class="font-mono text-xs uppercase tracking-widest text-ink/40">L6 / 6</p>
          <h2 class="font-display text-2xl text-ink">修饰 (Decoration)</h2>
    ...
    <section class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
      <div v-for="f in L6_SELECT_FIELDS" :key="f.key">
    ...
    <!-- 数值 slider 字段(腮红 / 眼影 / 彩绘) -->
    <section class="space-y-4">
      <div v-for="f in L6_SLIDER_FIELDS" :key="f.key">
