...
/**
 * Step 3 — L3 五官定位 (Features)
 *
 * 12 项:眼(3) + 鼻(3) + 唇(2) + 耳(2) + 人中 + 下巴
 * 数据来源:BlueprintContext (与 Step1/Step2 一致)
 */
...
if (!ctx) throw new Error('Step3Features must be inside BlueprintWizard (BlueprintKey not provided)');
const blueprintCtx = ctx;

const form = ref<L3Features>({ ...L3_DEFAULTS });
const saving = ref(false);
const lastSavedAt = ref<string | null>(null);

const draft = useBlueprintDraft<L3Features>({
  blueprintId: props.blueprintId,
  step: 3,
  serverData: ref<L3Features | null>(null),
  formData: form,
});
...
watch(
  () => blueprintCtx.blueprint.value?.layers.L3_features,
  (serverLayer) => {
    if (!serverLayer || draftRestored.value) return;
    form.value = { ...form.value, ...(serverLayer as unknown as L3Features) };
  },
  { immediate: true },
);
...
async function persist() {
  ...
  saving.value = true;
  try {
    await blueprintCtx.updateLayer(3, form.value as unknown as Record<string, unknown>);
...
const isLayerInferred = useStepInferred(blueprintCtx, 'L3_features');
</script>
  ...
  <article class="paper-grain rounded-md border border-ink/15 bg-cream p-6 shadow-paper">
    <header class="mb-4 border-b border-ink/10 pb-3">
      <div class="flex items-center justify-between">
        <div>
          <p class="font-mono text-xs uppercase tracking-widest text-ink/40">L3 / 12</p>
          <h2 class="font-display text-2xl text-ink">五官定位 (Features)</h2>
      ...
      >
        ...
        <span>本层 12 项由 AI 从参考图反推(角标标记);请逐项核对,首次编辑任一字段后,角标自动消失。</span>
      </div>
    </header>

    <!-- 枚举字段(眼型 / 鼻梁) -->
    <section class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
      <div v-for="f in L3_SELECT_FIELDS" :key="f.key">
    ...
    <section class="space-y-4">
      <div v-for="f in L3_SLIDER_FIELDS" :key="f.key">
